...
log(){
    echo "$(date "+%Y-%m-%d %H:%M:%S") [test-installer] -> $@"
}
...
if [[ "$OSTYPE" = "darwin"* ]]; then
    ...
    log "Contents of ${root_prefix}:"
    ls -al $root_prefix
    log "Contents of ${root_prefix}/uninstall-spyder.sh:"
    cat $root_prefix/uninstall-spyder.sh
    log "Contents of $HOME/.bashrc:"
    cat $HOME/.bashrc

    if [[ -e "$shortcut_path" ]]; then
        log "Contents of $shortcut_path/Contents/MacOS:"
        ls -al $shortcut_path/Contents/MacOS
        log "Contents of $shortcut_path/Contents/Info.plist:"
